...
public class Capitulo11n {
    ...
    public static void main(String[] args) {
        ...
        Map<Produto, BigDecimal> totalPorProduto =
                pagamentos.stream()
                          .flatMap(p -> p.getProdutos().stream())
                          .collect(
                                  groupingBy(
                                          identity(), Collectors.reducing(
                                                  BigDecimal.ZERO, Produto::getPreco, BigDecimal::add)));

        totalPorProduto.entrySet()
                       .stream()
                       .sorted(Map.Entry.comparingByValue())
                       .forEach(System.out::println);

        Map<Produto, BigDecimal> totalPorProduto2 =
                pagamentos.stream()
                          .flatMap(p -> p.getProdutos().stream())
                          .collect(toMap(Function.identity(), Produto::getPreco, BigDecimal::add));

        totalPorProduto2.entrySet()
                .stream()
                .sorted(Map.Entry.comparingByValue())
                .forEach(System.out::println);
