...
public class Capitulo11v {
    ...
    public static void main(String[] args) {
        ...
        Function<Pagamento, BigDecimal> decimalFunction =
                p -> p.getProdutos()
                      .stream()
                      .map(Produto::getPreco)
                      .reduce(BigDecimal.ZERO, BigDecimal::add);

        Map<YearMonth, BigDecimal> yearMonthBigDecimalMap =
                pagamentos.stream()
                          .collect(
                                  groupingBy(
                                          p -> YearMonth.from(
        ...
        yearMonthBigDecimalMap.entrySet().stream().forEach(System.out::println);
